...
export async function saveUser(input: User): Promise<Result<SavedUserAccount, UserSaveError>> {
...
export async function validateUsernameNotEmpty(input: User): Promise<Result<User, UserValidationError>> {
    await lastValueFrom(timer(50));
...
export function validateUsernameHasValidChars(input: User): Result<User, UserValidationError | UserError> {

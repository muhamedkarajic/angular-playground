 ...
 */
 ...
 const NG_DEV_MODE = false; // Not sure how to setup to be used in my project.
 ...
 export class MyIfDirective<T, U extends NgIterable<T> = NgIterable<T>> implements DoCheck {
   /**
    * The value of the iterable expression, which can be used as a
    * [template input variable](guide/structural-directives#shorthand).
    */
   @Input()
   set myIf(ngForOf: U & NgIterable<T> | undefined | null) {
   ...
   private _applyChanges(changes: IterableChanges<T>) {
    console.log('MyIfDirective(_applyChanges): ', changes);
    const val = this._ngForOf;
    if(val) {
      if(val instanceof Array && val.length !== val.filter(x => !!x).length)
    ...
    */
   static ngTemplateContextGuard<T, U extends NgIterable<T>>(dir: MyIfDirective<T, U>, ctx: any):
